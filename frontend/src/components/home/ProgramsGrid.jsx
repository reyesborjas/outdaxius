// ===============================
// src/components/home/ProgramsGrid.jsx
// ===============================
import React, { useEffect, useMemo, useState } from "react";
import useApiList, { normalizeGallery } from "./useApiList";

export default function ProgramsGrid({ asCarousel = false }) {
  const { data, loading } = useApiList("/programs/");
  ...
  const coverOf = (p) => {
    const g = normalizeGallery(p?.gallery);
  ...
  return (
    <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
      {items.map((p) => (
        <div key={p.id} className="col">
          <div className="h-100 border rounded-3 bg-white shadow-sm overflow-hidden">
            {coverOf(p) && (
              <img src={coverOf(p)} alt={p.title || "program"} style={{ width: "100%", height: 140, objectFit: "cover" }} />
            )}
            <div className="p-2">
      ...
      {items.length === 0 && <p className="text-muted mb-0">No programs found.</p>}
    </div>
  );
}
