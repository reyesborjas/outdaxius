...
def create_activity_schedule(
    ...
    if payload.program_schedule_id is None:
        ...
        # NOTE: do not gate this on `creator_team_id` being truthy -- an unaffiliated guide (no
        # team at all) has no home team to justify reuse under, so this must still run and deny
        # for them. check_can_reuse() already returns False when creator_team_id is None.
        if activity.team_id and activity.team_id != creator_team_id:
