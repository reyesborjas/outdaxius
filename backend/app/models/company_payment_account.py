...
class CompanyPaymentAccount(Base):
    ...
    __table_args__ = (
        # Kept in sync with migration 0006_demo_payment_provider, which widened the live
        # constraint to include 'demo' (app.services.payments.demo.DemoProvider). SQLAlchemy does
        # not enforce CheckConstraint at runtime, so the drift was invisible on insert -- but it
        # would have surfaced the moment anyone ran create_all() or autogenerated a migration.
        CheckConstraint(
            "provider IN ('flow', 'stripe', 'transbank', 'mercadopago', 'demo')",
